...
void gestureREC(){
 ...
 int moveoutput=0, n=5, counter=0;
 int i[n];
...
//  medianBlur(frame2, frame2, 7);
 ...
 { 
  vector<Point> poly;
  vector<int>  poly1;
  drawContours( frame1, contours, largestIndex, Scalar(0,0,0), 1, 8);

  convexHull(contours[largestIndex], poly, 1, 1);
...
// HuMoments(mu,hu);
 Point2f mc[n];
 mc[counter]= Point2f(mu[counter].m10/mu[counter].m00,mu[counter].m01/mu[counter].m00);
 avMc1.x+=mc[counter].x; avMc1.y+=mc[counter].y;
 convexityDefects(contours[largestIndex], poly1, conDef);
...
/*  line( frame, ptStart, ptFar, Scalar(200,182,98),3);
  ...
  if((depth>75)&&(dist(ptStart,ptEnd)>30)&&(ang(ptStart,ptEnd,ptFar)>0))
  {
...
// Point Start1, Start2, End1, End2; Start1.x=Start1.y=Start2.x=Start2.y=End1.x=End1.y=End2.x=End2.y=0; 
 ...
 if(flag1_1== true)
 {
  ...
  End1.x=avMc1.x; End1.y=avMc1.y; line(frame1, Start1, End1, Scalar(225,0,0), 2, 8,0); 
//  if((End1.x-Start1.x)>500) {moveoutput= 200; break;}
//  if((End1.x-Start1.x)<(-500)) {moveoutput= -200; break;}
 }
 ...
 if(flag2_1== true)
 {
  ...
  if( buffer==true) { Start2.x=list[0].x; Start2.y=list[0].y; End2.x=list[2].x; End2.y=list[2].y;}
 // if(fabs(dist(Start1, End1)/dist(Start2, End2))>1.5) { moveoutput=fabs(dist(Start1, End1)-dist(Start2, End2)) ; break;}
 // if(fabs(dist(Start1, End1)/dist(Start2, End2))<0.33){ moveoutput= -100; break;}
