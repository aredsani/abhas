...
all: random1.cpp texture.cpp shader.cpp LIB_shapes LIB_objLoader 
	$(CXX)  -I. $(CXXFlags3) random1.cpp texture.cpp shader.cpp objloader.o shapes.o   $(CXXFLAGS) -I/usr/local/include -lboost_thread -L/usr/local/lib  `pkg-config --cflags opencv` maya.cpp  `pkg-config --libs opencv` -o run
	rm -f *.o
